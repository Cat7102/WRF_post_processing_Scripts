...
fig_dpi=        150 #图片DPI
#图片的格式
ver_num,hor_num=    2,3 #一张图片显示子图的行数，列数
title_size=         10  #子图的标题
title_y=            1.03    #标题的高度
#地理信息
...
quiverkey_x,quiverkey_y=0.85,1.03       #quiverkey的相对位置，x和y
quiverkey_ws,quiverkey_text,quiverkey_size=4,'4m/s',8   #quiverkey的标注风速，标注字体和字体大小
#色块图例数据
label_opt=0                                     #是否采用自定义的色块位置，0表示是，1表示否
rect_place,rect_more='bottom',0.2               #设置空白子图的位置（bottom，top，right，left），数值表示空白多少，相对子图的比例
rect1,rect2,rect3,rect4=0.2, 0.1, 0.6, 0.02     #色块图例的位置,分别是水平位置，垂直位置，水平宽度，垂直宽度
hv_opt='horizontal'                               #图例垂直还是水平
label_text='温度坐标(℃)'                        #图例写什么字
c_label_size,c_tick_size=8,10                   #标签字体大小,刻度字体大小
